...
Flutter App (HTTPS REST/JSON)
  ...
  |  +-----------+    |
  |  | Patient   |    |  FHIR R4 write pipeline, 17 resource types
...
**Per-patient encryption:** Each patient's data is encrypted with a unique AES-256-GCM data encryption key (DEK), wrapped by a master key. Non-patient resources (Practitioner, Organization, etc.) use a system-level key. Destroying a patient's key renders their Git data permanently unreadable — crypto-erasure for privacy law compliance.
...
## Supported FHIR Resources (17 types)
...
- **Encryption at rest** — Per-patient AES-256-GCM envelope encryption. Master key wraps per-patient DEKs.
...
- **SQLite search index contains some PII** — Patient names, dates of birth, gender, and clinical codes are indexed for search. Deployed environments should use disk-level encryption (LUKS, FileVault) for defense in depth.
...
| 7 | SMART Scope | SMART v2 scope enforcement on FHIR endpoints |
